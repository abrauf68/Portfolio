        ...
        <div class="card mb-6">
            ...
            <div class="card-body pt-4">
                <form method="POST" action="{{ route('dashboard.skills.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="row p-5">
                        ...
                        <div class="mb-4 col-md-4">
                            ...
                            <select id="skill_type" name="skill_type" class="select2 form-select @error('skill_type') is-invalid @enderror" required>
                                ...
                                <option value="frontend"{{ old('skill_type') == 'frontend' ? 'selected' : '' }}>{{__('Frontend')}}</option>
                                <option value="backend"{{ old('skill_type') == 'backend' ? 'selected' : '' }}>{{__('Backend')}}</option>
                            </select>
                            @error('skill_type')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                        ...
                        <div class="mb-4 col-md-12">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description"
                                placeholder="{{ __('Enter skill description') }}" cols="30" rows="5" required>{{ old('description') }}</textarea>
                            @error('description')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                    ...
                    <div class="mt-2">
                        <button type="submit" class="btn btn-primary me-3">{{ __('Add Skill') }}</button>
                    </div>
                </form>
            </div>
            <!-- /Account -->
