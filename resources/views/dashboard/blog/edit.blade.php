        ...
        <div class="card mb-6">
            ...
            <div class="card-body pt-4">
                <form method="POST" action="{{ route('dashboard.blogs.update', $blog->id) }}" enctype="multipart/form-data">
                    ...
                    <div class="row p-5">
                                ...
                                @if (isset($blogCategories) && count($blogCategories) > 0)
                                    @foreach ($blogCategories as $blogCategory)
                                        <option value="{{ $blogCategory->id }}"
                                            {{ $blogCategory->id == old('blog_category_id', $blog->blog_category_id) ? 'selected' : '' }}>{{ $blogCategory->name }}
                                        </option>
                                    @endforeach
                                @endif
                            </select>
                        ...
                        <div class="mb-4 col-md-6">
                            ...
                            <input id="tags" name="tags"
                                class="form-control @error('tags') is-invalid @enderror" placeholder="Select tags" value="{{old('tags', $blog->tags)}}" />
                            @error('tags')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <div class="mb-4 col-md-12">
                            ...
                            <textarea class="form-control @error('content') is-invalid @enderror" id="content" name="content"
                                placeholder="{{ __('Enter blog content') }}" cols="30" rows="10">{{ old('content', $blog->content) }}</textarea>
                            @error('content')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                    ...
                    <div class="mt-2">
                        <button type="submit" class="btn btn-primary me-3">{{ __('Update Blog') }}</button>
                    </div>
                </form>
            </div>
            <!-- /Account -->
